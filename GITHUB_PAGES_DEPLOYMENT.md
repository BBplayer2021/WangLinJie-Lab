...
3. 配置 DNS（阿里云域名）：
   - 详细步骤请查看：[ALIYUN_DNS_SETUP.md](ALIYUN_DNS_SETUP.md)
   - 快速配置：
     1. 登录阿里云控制台 → **域名** → **域名解析 DNS**
...
**免费 GitHub 账户**：
- ✅ **公共仓库**：可以使用 GitHub Pages（完全免费）
- ❌ **私有仓库**：不能使用 GitHub Pages

**付费 GitHub 账户**（Pro/Team/Enterprise）：
- ✅ **公共仓库**：可以使用 GitHub Pages
- ✅ **私有仓库**：也可以使用 GitHub Pages（这是付费账户的优势之一）

**解决方案**：
- 如果仓库是私有的，有两个选择：
  1. **将仓库改为公开**（推荐，因为网站内容本身就是公开的）
  2. **升级到 GitHub Pro**（$4/月，可以使用私有仓库的 Pages）

### 其他限制

1. **构建时间限制**：每次构建最多 10 分钟
2. **存储限制**：1GB
3. **带宽限制**：每月 100GB（通常足够使用）
